...
export default function Design() {
  const courses = [
    { title: "UI/UX Fundamentals", lessons: 24, level: "Beginner" },
    { title: "Figma Masterclass", lessons: 30, level: "Intermediate" },
    { title: "Graphic Design Theory", lessons: 18, level: "Beginner" },
    { title: "3D Design with Blender", lessons: 36, level: "Intermediate" },
    { title: "Motion Design", lessons: 22, level: "Advanced" },
    { title: "Brand Identity Design", lessons: 28, level: "Intermediate" },
  ];
        ...
        <div className="relative z-10 text-center px-6">
          ...
          <FadeUp delay={200}>
            <p className="mt-4 text-white/40 max-w-xl mx-auto text-lg">
              Create beautiful, intuitive experiences that captivate and inspire.
            </p>
          </FadeUp>
        </div>
      </section>
        ...
        {courses.map((course, i) => (
          <FadeUp key={course.title} delay={i * 80}>
            <div className="group bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-500 hover:-translate-y-1">
              <div className="h-40 rounded-xl mb-5 bg-gradient-to-br from-purple-500/10 to-pink-500/5 border border-white/[0.04]" />
              <span className="text-xs text-purple-400 uppercase tracking-wide font-medium">{course.level}</span>
              <h3 className="font-semibold text-lg mt-1 text-white group-hover:text-purple-400 transition-colors">{course.title}</h3>
