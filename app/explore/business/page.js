...
export default function Business() {
  const courses = [
    { title: "Entrepreneurship 101", lessons: 22, level: "Beginner" },
    { title: "Business Strategy", lessons: 28, level: "Intermediate" },
    { title: "Financial Management", lessons: 30, level: "Intermediate" },
    { title: "Leadership & Management", lessons: 24, level: "Beginner" },
    { title: "Negotiation Skills", lessons: 16, level: "Intermediate" },
    { title: "Scaling Operations", lessons: 34, level: "Advanced" },
  ];
        ...
        <div className="relative z-10 text-center px-6">
          ...
          <FadeUp delay={200}>
            <p className="mt-4 text-white/40 max-w-xl mx-auto text-lg">
              Master the skills to lead teams, grow companies, and drive impact.
            </p>
          </FadeUp>
        </div>
      </section>
        ...
        {courses.map((course, i) => (
          <FadeUp key={course.title} delay={i * 80}>
            <div className="group bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-500 hover:-translate-y-1">
              <div className="h-40 rounded-xl mb-5 bg-gradient-to-br from-orange-500/10 to-amber-500/5 border border-white/[0.04]" />
              <span className="text-xs text-orange-400 uppercase tracking-wide font-medium">{course.level}</span>
              <h3 className="font-semibold text-lg mt-1 text-white group-hover:text-orange-400 transition-colors">{course.title}</h3>
